    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: Container(
          height: 400,
          color: Colors.white,
          child: Container(child: _TestFile(),),
        ),
      ),
...
//등록
class _TestFile extends StatefulWidget {
  const _TestFile({super.key});

  @override
  State<_TestFile> createState() => _TestFileState();
}

//할일정의
class _TestFileState extends State<_TestFile> {
  final TextEditingController _countController = TextEditingController();
  int totalOrderPrice = 0; //초기값

  late Future<List<TestVo>> shopListFuture;
  ...
  //리스트가져오기 dio통신
  Future<List<TestVo>> getShopList() async {
    try {
      /*----요청처리-------------------*/
      //Dio 객체 생성 및 설정
      var dio = Dio();
    ...
    //예시
      ...
      if (response.statusCode == 200) {
        ...
        List<TestVo> shopList = [];
        for (int i = 0; i < response.data["apiData"].length; i++) {
          print("반복확인");

          TestVo testVo = TestVo.fromJson(response.data["apiData"][i]);
